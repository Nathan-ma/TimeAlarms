...
/*
 2 July 2011 - replaced alarm types implied from alarm value with enums to make
 trigger logic more robust
             - this fixes bug in repeating weekly alarms - thanks to Vincent
 Valdy and draythomp for testing
*/
...
void AlarmClass::updateNextTrigger() {
  if (Mode.isEnabled) {
    ...
    if (Mode.alarmType == dtTimer) {
      // its a timer
      nextTrigger = time + value;  // add the value to previous time (this ensures delay
                                   // always at least Value seconds)
    }
  }
}
...
void TimeAlarmsClass::enable(AlarmID_t ID) {
  if (isAllocated(ID)) {
    if ((!(dtUseAbsoluteValue(Alarm[ID].Mode.alarmType) && (Alarm[ID].value == 0))) &&
      ...
      Alarm[ID].updateNextTrigger();  // trigger is updated whenever  this is
                                      // called, even if already enabled
    } else {
...
// returns true only if id is allocated and the type is a time based alarm,
// returns false if not allocated or if its a timer
...
AlarmID_t TimeAlarmsClass::getTriggeredAlarmId() const {
  if (isServicing) {
    return servicedAlarmId;  // new private data member used instead of local
                             // loop variable i in serviceAlarms();
  } else {
    return dtINVALID_ALARM_ID;  // valid ids only available when servicing a
                                // callback
  }
}
...
// returns isServicing
bool TimeAlarmsClass::getIsServicing() const { return isServicing; }
...
AlarmID_t TimeAlarmsClass::create(
    time_t value, OnTick_t onTickHandler, uint8_t isOneShot, dtAlarmPeriod_t alarmType) {
  if (!((dtIsAlarm(alarmType) && now() < SECS_PER_YEAR) ||
        (dtUseAbsoluteValue(alarmType) && (value == 0)))) {
