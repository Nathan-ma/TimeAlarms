...
typedef struct {
  ...
  uint8_t isOneShot : 1;  // the timer will be de-allocated after trigger is
                          // processed
} AlarmMode_t;
...
// macro to return true if the given type is a time based alarm, false if timer
// or not allocated
...
class TimeAlarmsClass {
 ...
 public:
  ...
  AlarmID_t alarmOnce(
      const timeDayOfWeek_t DOW, const int H, const int M, const int S, OnTick_t onTickHandler) {
  ...
  // utility methods
  uint8_t getDigitsNow(
      dtUnits_t Units) const;  // returns the current digit value for the given time unit
  ...
  void write(
      AlarmID_t ID,
      time_t value);                // write the value (and enable) the alarm with the given ID
 ...
 private:  // the following methods are for testing and are not documented as part
           // of the standard library
#endif
  uint8_t count() const;  // returns the number of allocated timers
  void writeNextTrigger(AlarmID_t ID, time_t time) {
    if (isAllocated(ID)) {
      Alarm[ID].nextTrigger = time;
    }
  }
  ...
  bool isAlarm(AlarmID_t ID) const;           // returns true if id is for a time based alarm,
                                              // false if its a timer or not allocated
};
